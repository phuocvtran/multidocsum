...
class Scorer:
    ...
    @staticmethod
    def get_first_sent_overlap_score(data_list: List[str], pos_list: List[int], doc_list: List[str], ngram_range: Tuple[int, int]=(1, 1), max_features: int=300, func: Callable=np.dot) -> List[float]:
        df = pandas.DataFrame({'data': data_list, 'pos': pos_list, 'doc': doc_list})
    ...
    def scoring_sentences(raw_data: Dict[str, str], processed_data: Dict[str, str], wc: float=1.0, wp: float=1.0, wf: float=1.0, n_centroid: int=20, C_max:int=100, ngram_range: Tuple[int, int]=(1, 1), max_features: int=300, func: Callable=np.dot) -> pandas.DataFrame:
        ...
        pos_score = Scorer.get_pos_score(data_frame.doc.to_list(), data_frame.pos.to_list(), C_max=C_max)
        data_frame['pos_score'] = pos_score
        ovl_score = Scorer.get_first_sent_overlap_score(data_frame.processed.to_list(), data_frame.pos.to_list(), data_frame.doc.to_list(), ngram_range=ngram_range, max_features=max_features, func=func)
        ...
        data_frame = Scorer.get_redundancy_score(data_frame, compression_rate=compression_rate)
